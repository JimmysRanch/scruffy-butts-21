...
## Design Direction
The design should evoke luxury, depth, and premium quality through a futuristic liquid glass aesthetic with true dimensional depth. The interface features independent floating panes of illuminated frosted glass, each with sophisticated blur effects and refraction-inspired gradients. Rich jewel-toned backgrounds (sapphire, amethyst, rose-gold) create ambient light diffusion, while each widget is tinted differently (lavender, turquoise, rose, gold, emerald) with soft glowing edges. Micro-charts with radiant strokes, simulated key lighting from top-left creating highlights, and soft underglows beneath each widget produce the illusion of floating depth. Every surface feels like premium liquid glass suspended in ambient light, creating an immersive, dimensional, and calm experience that's futuristic and refined.

## Color Selection
Jewel-toned ambient gradient palette with liquid glass refraction - Creating a luxurious, dimensional experience with true depth and light simulation inspired by holographic UIs and ambient refraction.

- **Background Gradient**: Sapphire to Amethyst to Rose-Gold radial gradient (oklch(0.25 0.15 260) → oklch(0.08 0.04 340) → oklch(0.06 0.03 20)) - Creates ambient liquid light environment with gentle diffusion
- **Primary Color**: Luminous Purple (oklch(0.60 0.20 280)) - Core brand illumination with soft glow effects
- **Secondary Colors**: Multi-hued glass variants - Lavender (290°), Turquoise (200°), Rose (340°), Gold (60°), Emerald (160°) - Each widget has unique tint with matching underglow
- **Accent Color**: Radiant Magenta (oklch(0.65 0.22 310)) - Glowing highlights for interactive elements with drop-shadow illumination
- **Glass Refraction**: 28-32% opacity backgrounds, 20-24px blur, gradient borders with top-edge highlights simulating key light refraction through curved glass
- **Lighting System**: Top-left key light creates highlights on upper widget edges, soft colored underglows (4-16px blur) matching widget tints create floating depth, micro-reflections and inner gradients mimic light bending
- **Foreground/Background Pairings**:
  - Background (Deep Sapphire oklch(0.08 0.06 280)): White 85% opacity (oklch(0.98 0.01 280 / 0.85)) - Crisp, legible, premium ✓
  - Glass Widgets (Various Tints ~22% opacity): White 90% for headers (oklch(0.98 0.01 / 0.90)), White 60% for body (oklch(0.98 0.01 / 0.60)) - Enhanced by blur and glow ✓  
  - Primary (Luminous Purple oklch(0.60 0.20 280)): White text with glow effects - Ratio 8.1:1 ✓
  - Accent (Radiant Magenta oklch(0.65 0.22 310)): White text with shadow glow - Ratio 7.5:1 ✓
...
- **Purposeful Meaning**: Fluid transitions with gentle scale (1.02x), subtle glow intensification, and refined blur shifts communicate premium quality and cutting-edge technology. Hover states increase underglow intensity and border luminosity, simulating light catching on glass surfaces
- **Hierarchy of Movement**: Micro-interactions on widgets with scale and glow (400ms cubic-bezier), elegant sheet transitions with backdrop blur fade-in (300ms), form focus states with radiant ring glow (200ms), chart animations with progressive reveal and soft stroking effects

## Component Selection
- **Components**: Glass-morphic cards with unique color tints for appointments/customers/transactions/settings/staff/inventory items/report metrics/dashboard stats, each with top-edge highlights and bottom underglows. Multi-view calendar (day/week/month/list) with tabs for scheduling, Forms with radiant focus rings, Tables with frosted row backgrounds, Glowing badges for appointment status, Shopping cart with glass panels, Illuminated switches and toggles, Tabs with light-catching active states, Dialogs with deep backdrop blur, Sheets with gradient borders, Filters with soft glow, Micro-charts (progress arcs, bar fills, glowing sparklines) with radiant strokes matching widget accent colors
- **Customizations**: Liquid glass widgets system with 5 color variants (lavender/turquoise/rose/gold/emerald), each with custom CSS variables for gradients, borders, shadows, and underglows. Advanced calendar with floating glass date cells, Enhanced appointment status with glowing badges, Integrated checkout sheets with consistent glass aesthetic, Conflict detection with pulsing warning glow, POS cart with translucent item cards, Dashboard stat widgets with embedded micro-charts (sparklines with 2.5px strokes, mini bar charts with glow, progress arcs with radial illumination), Reports with holographic KPI cards and luminous trend indicators
- **States**: Buttons show pulsing glow on loading, Inputs have radiant focus rings with accent color glow, Appointments display status with colored illuminated badges, Glass widgets scale and intensify underglow on hover (translateY(-4px) with increased shadow spread), Charts animate strokes with progressive glow reveal, Metric cards show trend with glowing arrows, Dashboard widgets have smooth hover elevation with enhanced border luminosity
- **Icon Selection**: Phosphor icons with duotone weight for depth - All icons enhanced with matching drop-shadow glows (0_0_4-8px with widget's accent color). Calendar/CalendarBlank for scheduling, User/UserCircle for customers, Dog for pets, Scissors/Package for services, Phone/Envelope for contact, CashRegister/CreditCard for POS, Money/TrendUp/TrendDown with glows for financial indicators, Gear for settings, Bell/BellRinging with pulsing glow for notifications, Clock/ClockCounterClockwise for time tracking, CheckCircle/XCircle with status-colored glows, ChartBar/ChartLineUp with data visualization glow effects
...
- **Mobile**: Glass navigation adapts with reduced blur on low-power devices, Widgets stack vertically maintaining individual color tints and glows, Touch targets expand to 44px minimum with generous tap zones, Sheets and dialogs maintain full glass treatment with optimized blur for performance, Micro-charts scale proportionally preserving glow effects